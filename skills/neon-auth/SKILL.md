...
name: neon-auth
description: >-
  Set up Neon Auth (Managed Better Auth) for login, users, sessions, and social
  sign-in, or choose self-managed Better Auth when a required plugin is outside
  Managed Auth support. Use for authentication on Neon, sign-up, sign-in,
  password reset, email OTP, magic links, organizations, phone OTP, trusted
  domains, invalid domain, and @neondatabase/auth. Also use for the auth APIs
  in @neondatabase/neon-js, Better Auth plugin support, MFA, passkeys, SSO,
  API keys, and MCP OAuth when choosing identity for a Neon app or Function.
metadata:
  parent: neon
...
Start here for undecided login. Keep Clerk or another working provider unless the user asks to migrate. A needed Better Auth plugin that Managed Auth does not support is the signal to run self-managed Better Auth instead.
...
Then implement login: [references/managed-auth.md](references/managed-auth.md). Done means sign-up, sign-in, sign-out, session display, and protected access work, including error and loading states.
...
| Organization | Partial, Beta | Members, invitations, owner/admin/member. No Teams, server hooks, custom roles/permissions, or dynamic access control. |
...
| Phone Number | Supported with constraints | Browser client: existing users link a number, then sign in; no phone-first signup; own SMS webhook; custom UI. **Next.js:** `createNeonAuth` server proxy in inspected `@neondatabase/auth` has no `phoneNumber` namespace. Confirm the installed SDK's server endpoint tree before wiring phone OTP through the Next same-origin proxy. If the namespace is missing, stop and report that boundary; do not infer a bypass or switch identity providers. https://neon.com/docs/auth/guides/plugins/phone-number.md |
| MFA / Two-Factor | Roadmap | Unavailable today. |
| Passkey, API Key, Generic OAuth, One Tap, Multi Session | Not exposed by Managed SDK/UI | Need one of these: self-managed Better Auth. Generic OAuth is not the same as Google/GitHub/Vercel social sign-in. |
| MCP / OAuth Provider | Use self-managed Better Auth | Third-party MCP clients self-authorizing against your server. See `neon-functions` and [references/self-managed.md](references/self-managed.md). |
| SSO / SAML | Not listed or exposed | Upstream Better Auth documents it. No Neon delivery date found. |
