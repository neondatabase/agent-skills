...
- Declare `vector` columns in the initial `CREATE TABLE`. Never add them with a later `ALTER TABLE`.

Drizzle cannot express `CREATE EXTENSION`, the `lakebase_ann` / `lakebase_bm25` access methods, or a generated `tsvector` column, and hence they go in a custom migration.
...
Define the columns Drizzle can express. Declare `embedding` in the initial `CREATE TABLE`. Never add it with a later `ALTER TABLE`: Drizzle mis-quotes `vector(1536)` as a literal type when altering an existing table.
...
Source: [Schema migration with Lakebase Postgres and Drizzle ORM](https://neon.com/docs/guides/drizzle-migrations).
